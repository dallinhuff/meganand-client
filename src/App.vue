...
<template>
  <div class="drawer">
    <input id="toggle-nav-drawer" type="checkbox" v-model="expanded" class="drawer-toggle"/>
    <div class="drawer-content flex flex-col">
      ...
      <div class="w-full navbar">
        ...
        <router-link
          to="/"
          class="flex-1 px-2 mx-2 text-xl font-serif italic">
          {{ title }}
        </router-link>
    ...
    <div class="drawer-side">
      ...
      <ul class="menu p-4 w-80 min-h-full bg-base-100">
        <li v-for="option in tabOptions" :key="option.id">
          <router-link :to="option.id" @click="expanded = false">{{option.label}}</router-link>
        </li>
      </ul>
    </div>
  </div>
